...
export function registerSpecialtiesPage() {
    window.Alpine.data('specialtiesPage', (config) => ({
        ...
        applySearch(detail) {
            ...
            delete this.query.specialties_page;
            this.refreshTable();
        },

        async refreshTable() {
            const target = document.getElementById('specialties-table-body');
            if (!target) {
                return;
            }
            ...
            try {
                ...
                console.error('Recherche spécialités :', error);
            } finally {
                this.loading = false;
