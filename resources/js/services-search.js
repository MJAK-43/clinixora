...
export function registerServicesPage() {
    window.Alpine.data('servicesPage', (config) => ({
        ...
        applySearch(detail) {
            ...
            delete this.query.services_page;
            this.refreshTable();
        },

        async refreshTable() {
            const target = document.getElementById('services-table-body');
            if (!target) {
                return;
            }
            ...
            try {
                ...
                console.error('Recherche services :', error);
            } finally {
                this.loading = false;
