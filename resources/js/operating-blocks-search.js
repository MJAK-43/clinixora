...
export function registerOperatingBlocksPage() {
    window.Alpine.data('operatingBlocksPage', (config) => ({
        ...
        applySearch(detail) {
            ...
            delete this.query.operating_blocks_page;
            this.refreshTable();
        },

        async refreshTable() {
            const target = document.getElementById('operating-blocks-table-body');
            if (!target) {
                return;
            }
            ...
            try {
                ...
                console.error('Recherche blocs opératoires :', error);
            } finally {
                this.loading = false;
