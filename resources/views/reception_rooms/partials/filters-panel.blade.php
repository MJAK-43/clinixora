<aside class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm" aria-labelledby="reception-rooms-filters-heading">
    <h2 id="reception-rooms-filters-heading" class="text-sm font-semibold text-slate-900">Filtres</h2>

    <form method="GET" action="{{ route('reception_rooms.index') }}" class="mt-4 space-y-4">
        <div>
        ...
        <div class="flex gap-2 pt-2">
            <a href="{{ route('reception_rooms.index', ['per_page' => $perPage]) }}" class="inline-flex flex-1 items-center justify-center gap-1 rounded-lg border border-slate-200 px-3 py-2 text-xs font-medium text-slate-600 hover:bg-slate-50">Réinitialiser</a>
