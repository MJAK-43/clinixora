@php
    $deleteRedirectQuery = collect(request()->query())
        ->except(['edit_reception_room', 'create_reception_room'])
        ->all();
@endphp
...
<div class="min-h-0 flex-1 overflow-auto">
    <table class="min-w-full text-sm">
        ...
        <tbody class="divide-y divide-slate-100">
                ...
                <tr class="transition hover:bg-slate-50">
                    <td class="px-4 py-3">
                        <span class="inline-flex h-9 w-9 items-center justify-center rounded-lg bg-sky-50">
                            @include('reception_rooms.partials.icon', ['icon' => $room->icon])
                        </span>
                    </td>
                    ...
                    <td class="px-4 py-3">
                        @include('reception_rooms.partials.row-actions', [
                            'editUrl' => route('reception_rooms.index', array_merge(request()->query(), ['edit_reception_room' => $room->id])),
                            'deleteAction' => route('reception_rooms.destroy', $room),
            ...
            @empty
                <tr>
                    <td colspan="6" class="px-4 py-10 text-center text-slate-500">Aucune salle d'accueil trouvée.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
...
@include('reception_rooms.partials.pagination', [
    'paginator' => $rooms,
    'label' => 'salles d\'accueil',
    'perPage' => $perPage,
])
